...
fun Spinner(list : List<String>,
    ...
    Box {
        Column {
            ...
            DropdownMenu(modifier = Modifier.fillMaxWidth(),
                ...
                list.forEach {
                    ...
                    text = { Text(text = entry, modifier = Modifier.wrapContentWidth().align(Alignment.Start)) },
                    onClick = {
                        selected = entry
                        expanded = false
        ...
        Spacer(
            modifier = Modifier.matchParentSize().background(Color.Transparent).padding(10.dp).clickable(onClick = {expanded = !expanded})
        )
    }
}

@Composable
fun Button(selectedItem: String, onSelectionChanged: (String) -> Unit, onClick: () -> Unit) {
    FilledTonalButton(onClick = {
        if (selectedItem == "Halloween") {
